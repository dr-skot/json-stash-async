...
describe("stash", () => {
  ...
  it("handles primitives just like JSON.stringify", async () => {
    ...
    for (const input of inputs) {
      expect(await stashAsync(input)).toEqual(JSON.stringify(input));
      expect(await unstashAsync(await stashAsync(input))).toEqual(input);
    }
  });
...
describe("addClasses", () => {
  ...
  it("adds a class to the default set", async () => {
    class Agent {
      first: string;
      last: string;
    ...
    const stashed = await stasher1.stashAsync(new Agent("James", "Bond"));
    const unstashed1 = await stasher2.unstashAsync(stashed);
    expect(unstashed1.introduce).not.toBeDefined();

    stasher2.addClasses(Agent);
    const unstashed2 = await stasher2.unstashAsync(stashed);
    ...
    const unstashed3 = await stasher2.unstashAsync(stashed);
    expect(unstashed3.introduce).not.toBeDefined();
  });

  it("allows you to name classes", async () => {
    class Agent {
      first: string;
      last: string;
      constructor(first: string, last: string) {
    ...
    const stashed = await stasher.stashAsync(new Agent("James", "Bond"));
    expect(stashed).toContain('"$type":"SpecialAgent"');

    const unstashed = await stasher.unstashAsync(stashed);
...
describe("addSerializers", () => {
  ...
  it("adds a serializer to the default set", async () => {
    class Agent {
      first: string;
      last: string;
      constructor(first: string, last: string) {
    ...
    let agent = await stasher2.unstashAsync(
      await stasher1.stashAsync(new Agent("James", "Bond")),
    );
    expect(agent.introduce).not.toBeDefined();

    let stashed = await stasher1.stashAsync(new Agent("James", "Bond"), [
      agentSerializer,
    ]);
    agent = await stasher2.unstashAsync(stashed, [agentSerializer]);
    expect(agent.introduce()).toBe("My name is Bond. James Bond.");

    stasher2.addSerializers(agentSerializer);
    agent = await stasher2.unstashAsync(stashed);
    expect(agent.introduce()).toBe("My name is Bond. James Bond.");
  });
});

describe("stashers", () => {
  it("have different serializer sets", async () => {
    const stasher1 = getStasher();
    const stasher2 = getStasher();
    ...
    expect(await stasher1.stashAsync(d)).toBe(
      '{"$type":"Date","data":"2020-01-01T00:00:00.000Z"}',
    );
    expect(await stasher2.stashAsync(d)).toBe(
      '{"$type":"Date","data":"not supported"}',
    );
  });
});
...
describe("identical objects", () => {
  ...
  it("can handle nested plain objects that are recognized by a serializer", async () => {
    ...
    expect(await stashAsync(q)).toBe('{"$type":"Queue","data":[{"$ref":"$"}]}');

    const unstashed = await unstashAsync(await stashAsync(q));
